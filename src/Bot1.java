...
public class Bot1 implements BotAPI {
	...
	public String getCommand() {
		...
		String lettersInFrameOG = me.getFrameAsString().replaceAll(", ", "").replaceAll("\\]", "").replaceAll("\\[",
				"");
		setMyLetter(lettersInFrameOG.replace("_", "E")); // E is the most common letter in English so we replace any
															// blank tiles with an E
		String command = "";
		...
		if (turnCount == 0) { // At beginning of game set name and initialize list of real words (dictionary)
			...
			/*
			 * At the beginning of each turn check if should challenge (except first turn as
			 * it was not letting us place on board if word removed)
			 */
			boolean challenge = false;

			/*
			 * Our bot never places fake words so if any words on the board aren't real then
			 * challenge
			 */
			...
			if (!challenge) {
				if (me.getFrameAsString().isEmpty()) // At the end of game, frame may be empty so pass
				{
					...
					for (int i = 0; i < wordsOnBoard.size(); i++) {
						findrealPlayableWords(wordsOnBoard.get(i).getLetters(), myLetter, wordsOnBoard.get(i).getRow(),
								wordsOnBoard.get(i).getColumn(), wordsOnBoard.get(i).isHorizontal());
					}
					...
					if (realPlayableWords.isEmpty()) {
						...
						int realWordIndex = realPlayableWordsScore.indexOf(Collections.max(realPlayableWordsScore)); // Get
																														// the
																														// word
																														// with
																														// the
						...
						if (lettersInFrameOG.contains("_")
							...
							if (lettersInFrameOG.contains("_")
								...
								command = colChar + "" + (row + 1 - rowOffset) + " " + direction + " "
										...
										+ " E";
								System.out.println("Blank played");
							} else
								command = colChar + "" + (row + 1 - rowOffset) + " " + direction + " "
